...
export const Keyboard = {
	...
	addElements() {
		...
		keyboard.className = 'keyboard hide-keyboard'
		text.className = 'text'
		text.innerHTML = 'Change language click (win)'
		document.body.append(keyboard);
		document.querySelector('.keyboard').appendChild(text)
	},
	...
	animation(el) {
		...
		element.style.backgroundColor = 'rgb(214, 44, 26)';
	...
	printMassage(btn) {
		...
		buttons.forEach((el) => {
			if (el.id === btn) {
				output.setRangeText(el.innerHTML, output.selectionStart, output.selectionEnd, 'end')
			}
		})
	},
	...
	addClickArrowsHandler(btn) {
		const output = document.querySelector('.search-input');
		if (btn === 'ArrowLeft') {
			output.setRangeText('', output.selectionStart - 1, output.selectionEnd - 1, 'end')
